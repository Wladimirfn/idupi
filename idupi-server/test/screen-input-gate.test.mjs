...
function startServer(port, extraEnv = {}) {
  const child = spawn(process.execPath, [join(repoRoot, "idupi-server", "index.mjs")], {
...
async function waitForServer(port, timeoutMs = 20_000, describeChild = null) {
  ...
  const tail = describeChild ? `\nchild output:\n${describeChild()}` : "";
  throw new Error("server did not start in time" + tail);
}
